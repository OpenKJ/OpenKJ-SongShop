    ...
    ui(new Ui::DlgSongShopPurchase)
{
    ...
    if (!QDir(settings->storeDownloadDir()).exists())
        QDir().mkdir(settings->storeDownloadDir());
    setupDone = false;
    ui->setupUi(this);
    ...
    ui->lineEditDirectory->setText(settings->storeDownloadDir());
    shop = songShop;
    knLoginTest = false;
...
void DlgSongShopPurchase::on_pushButtonBrowse_clicked()
{
    ...
    if (fileName != "")
    {
        ...
        ui->lineEditDirectory->setText(fileName + QDir::separator());
    }
}
